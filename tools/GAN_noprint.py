...
class GAN(object):
    ...
    def convolutional_layer(self, outChannels, _patchShape, activation_function=tf.nn.tanh, stride=[1], name="conv",
                            reuse=False, batch_norm=False, train=None, in_layer=None):
        if in_layer == None:
            in_layer = self.layer
        with tf.variable_scope(name):
            ...
            if len(_patchShape) == 2:
                ...
                W = self.weight_variable([_patchShape[0], _patchShape[1], _patchShape[2], inChannels, outChannels],
                                         name=name)
            ...
            if batch_norm:
                ...
                self.layer = tf.contrib.layers.batch_norm(self.layer, decay=self.bn_decay, scale=True,
                                                          scope=tf.get_variable_scope(), reuse=reuse, fused=False,
                                                          is_training=train)
            ...
            # user output
            # if activation_function:
            #     print("Convolutional Layer \'{}\' {} ({}) : {}, BN:{}".format(name, W.get_shape(),
            #                                                                   activation_function.__name__,
            #                                                                   self.layer.get_shape(), batch_norm))
            # else:
            #     print("Convolutional Layer \'{}\' {} ({}) : {}, BN:{}".format(name, W.get_shape(), 'None',
            #                                                                  self.layer.get_shape(), batch_norm))
    ...
    # returns both normalized and linearized versions
    def residual_block(self, s1, s2, filter, activation_function=tf.nn.tanh, name="RB", reuse=False, batch_norm=False,
                       train=None, in_layer=None):
        ...
        A, _ = self.convolutional_layer(s1, filter, activation_function, stride=[1], name=name + "_A",
                                        in_layer=in_layer, reuse=reuse, batch_norm=batch_norm, train=train)
        B, _ = self.convolutional_layer(s2, filter, None, stride=[1], name=name + "_B", reuse=reuse,
                                        batch_norm=batch_norm, train=train)
        # shortcut connection
        s, _ = self.convolutional_layer(s2, filter1, None, stride=[1], name=name + "_s", in_layer=in_layer, reuse=reuse,
                                        batch_norm=batch_norm, train=train)
    ...
    def max_pool(self, window_size=[2], window_stride=[2]):
        if len(self.layer.get_shape()) == 4:
            self.layer = tf.nn.max_pool(self.layer, ksize=[1, window_size[0], window_size[0], 1],
                                        strides=[1, window_stride[0], window_stride[0], 1], padding="VALID")
        elif len(self.layer.get_shape()) == 5:
            self.layer = tf.nn.max_pool3d(self.layer, ksize=[1, window_size[0], window_size[0], window_size[0], 1],
                                          strides=[1, window_stride[0], window_stride[0], window_stride[0], 1],
                                          padding="VALID")
        # user output
    ...
    def avg_pool(self, window_size=[2], window_stride=[2]):
        if len(self.layer.get_shape()) == 4:
            self.layer = tf.nn.avg_pool(self.layer, ksize=[1, window_size[0], window_size[0], 1],
                                        strides=[1, window_stride[0], window_stride[0], 1], padding="VALID")
        elif len(self.layer.get_shape()) == 5:
            self.layer = tf.nn.avg_pool3d(self.layer, ksize=[1, window_size[0], window_size[0], window_size[0], 1],
                                          strides=[1, window_stride[0], window_stride[0], window_stride[0], 1],
                                          padding="VALID")
        # user output
    ...
    def avg_depool(self, window_size=[2, 2], window_stride=[2, 2]):
        ...
        if is3D:  # recover D dimension
            ...
            self.layer = tf.image.resize_images(self.layer, [int(s[1] * window_size[0]), int(s[2])],
                                                0)  # 0 = ResizeMethod.BILINEAR
            self.layer = tf.reshape(self.layer,
                                    [-1, int(s[1] * window_size[0]), int(s[2]), int(s[3]), int(s[4])])  # NDHWC
    ...
    # _patchShape: 2D: [H,W]; 3D: [D,H,W]
    # stride 3D: if 1D: [DHW], if 2D:[D,HW], if 3D:[D,H,W]
    def deconvolutional_layer(self, outChannels, _patchShape, activation_function=tf.nn.tanh, stride=[1], name="deconv",
                              reuse=False, batch_norm=False, train=None, init_mean=0., strideOverride=None):
        if init_mean == 1.:
        ...
        with tf.variable_scope(name):
            ...
            if len(_patchShape) == 2:
                ...
                W = self.weight_variable([_patchShape[0], _patchShape[1], outChannels, inChannels], name=name,
                                         init_mean=init_mean)
                self.layer = self.deconv2d(self.layer, W,
                                           [self.batch_size, int(shape[1] * stride[0]), int(shape[2] * stride[1]),
                                            outChannels], dcStride)
            ...
            if len(_patchShape) == 3:
                ...
                # create a weight matrix
                W = self.weight_variable([_patchShape[0], _patchShape[1], _patchShape[2], outChannels, inChannels],
                                         name=name, init_mean=init_mean)
                self.layer = self.deconv3d(self.layer, W,
                                           [self.batch_size, int(shape[1] * stride[0]), int(shape[2] * stride[1]),
                                            int(shape[3] * stride[2]), outChannels], dcStride)
            ...
            if len(_patchShape) == 2:
                self.layer = tf.reshape(self.layer,
                                        [-1, int(shape[1] * stride[0]), int(shape[2] * stride[1]), outChannels])
            if len(_patchShape) == 3:
                self.layer = tf.reshape(self.layer, [-1, int(shape[1] * stride[0]), int(shape[2] * stride[1]),
                                                     int(shape[3] * stride[2]), outChannels])
            ...
            # user output
            # if activation_function:
            #     # print("Deconvolutional Layer \'{}\' {} ({}): {}, BN:{}".format(name, W.get_shape(),
            #     #                                                                activation_function.__name__,
            #     #                                                                self.layer.get_shape(), batch_norm))
            # else:
            #     print("Deconvolutional Layer \'{}\' {} ({}): {}, BN:{}".format(name, W.get_shape(), 'None',
            #                                                                    self.layer.get_shape(), batch_norm))
            return self.layer, layer_lin
